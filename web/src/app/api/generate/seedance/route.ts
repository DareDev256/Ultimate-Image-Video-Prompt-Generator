...
// Polling can take 30s–5min; bump server timeout above the platform default.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Pro = full-quality model; Fast = $0.022/sec budget tier.
    const model = tier === 'fast'
      ? 'bytedance/seedance-2.0/fast/text-to-video'
      : 'bytedance/seedance-2.0/text-to-video';

    const { output, requestId } = await falQueueGenerate<SeedanceInput, SeedanceOutput>({
      model,
    ...
    return NextResponse.json({
      ...
      duration,
      seed: output.seed,
      model: tier === 'fast' ? 'Seedance 2.0 Fast' : 'Seedance 2.0 Pro',
    });
  } catch (error) {
